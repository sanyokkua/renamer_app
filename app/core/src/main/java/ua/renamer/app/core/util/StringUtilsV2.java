...
/**
 * V2 Utility class providing various string manipulation methods.
 * This class contains methods to convert strings to different cases
 * and to perform basic string checks.
 *
 * <p>This is a V2-specific copy independent from V1 utilities.</p>
 */
@Slf4j
...
public class StringUtilsV2 {
    ...
    public static String toPascalCase(final String inputString) {
        ...
        return separateWords.stream().map(StringUtilsV2::capitalize).collect(Collectors.joining());
    }
    ...
    public static String toTitleCase(final String inputString) {
        ...
        return separateWords.stream().map(StringUtilsV2::capitalize).collect(Collectors.joining(SPACE_SYMBOL));
    }
